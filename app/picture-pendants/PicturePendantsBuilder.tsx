...
export default function PicturePendantsBuilder({ accountSlug, backHref = "/design" }: { accountSlug?: string; backHref?: string } = {}) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (accountSlug) form.append("accountSlug", accountSlug);
      ...
      const response = await fetch("/api/picture-requests", {
        ...
        ...(upload
          ? {
              ...
              body: JSON.stringify({ userId: "demo", accountSlug, styleId: activeStyle.id, primaryMetal: goldColor, imageUpload: upload })
            }
          : { body: form })
      });
