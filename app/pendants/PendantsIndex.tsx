...
export default function PendantsIndex({ basePath }: { basePath?: string } = {}) {
  ...
  const visibleCards = cards.map(card => {
    ...
    const hrefMap: Record<string, string> = {
      "/name": basePath ? `${pendantBase}/icedout` : "/name",
      "/picture-pendants": basePath ? `${basePath}/picture-pendants` : "/picture-pendants",
      "/pendants/nameplates": `${pendantBase}/nameplates`,
      "/pendants/logo": `${pendantBase}/logo`,
    };
    return { ...card, href: hrefMap[card.href] ?? card.href };
  });
  return (
                ...
                <div className="relative aspect-square w-full overflow-hidden rounded-[22px] bg-black">
                  {card.thumb ? (
                    <Image
                      ...
                      className={`${card.id === "logo" ? "p-5 sm:p-6" : ""} object-contain object-center`}
